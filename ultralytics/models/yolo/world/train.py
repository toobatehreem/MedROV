...
class WorldTrainer(DetectionTrainer):
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides: Optional[Dict[str, Any]] = None, _callbacks=None):
        ...
        try:
            ...
            raise ImportError(
                "open-clip-torch is required for BiomedCLIP. Install with: pip install open-clip-torch"
            ) from e
        self.clip = open_clip
        ...
        self.text_model, _, _ = self.clip.create_model_and_transforms(
            "hf-hub:microsoft/BiomedCLIP-PubMedBERT_256-vit_base_patch16_224"
        )
        self.text_model = self.text_model.to(model_device).eval()
        for p in self.text_model.parameters():
            p.requires_grad_(False)

        self.tokenizer = self.clip.get_tokenizer(
            "hf-hub:microsoft/BiomedCLIP-PubMedBERT_256-vit_base_patch16_224"
        )
    ...
    def generate_text_embeddings(self, texts: List[str], batch: int, cache_dir: Path) -> Dict[str, torch.Tensor]:
        ...
        with torch.inference_mode():
            feats = self.text_model.encode_text(tokens)  # [N, C]
            feats = F.normalize(feats, p=2, dim=-1).detach().cpu().float()

        # Map each text to its embedding row; no squeeze!
        txt_map = {t: feats[i] for i, t in enumerate(texts)}
    ...
    def preprocess_batch(self, batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        txt_feats = torch.stack([self.text_embeddings[text] for text in texts]).to(batch["img"].device, batch["img"].dtype)
