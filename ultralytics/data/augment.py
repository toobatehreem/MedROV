...
class Mosaic(BaseMixTransform):
    ...
    def _mosaic4(self, labels):
        ...
        for i in range(4):
            ...
            img = labels_patch["img"]
            img_size = img.shape
            img_name = labels_patch["im_file"]
            concat_images.append(img_name)
            h, w = labels_patch.pop("resized_shape")
            ...
            mosaic_metadata.append({
                ...
                "mosaic_coords": (x1a, y1a, x2a, y2a),
                "img_size": img_size
            })
        mosaic = {"mosaic" : True}
        final_labels = self._cat_labels(mosaic_labels)
        final_labels["img"] = img4
        final_labels["metadata"] = mosaic_metadata
        final_labels["flags"] = mosaic
        return final_labels
...
class RandomFlip:
    ...
    def __call__(self, labels):
        ...
        if self.direction == "vertical" and random.random() < self.p:
            vertical_flip = {"vertical_flip": True}
            labels["flags"].update(vertical_flip)
            img = np.flipud(img)
            instances.flipud(h)
        if self.direction == "horizontal" and random.random() < self.p:
            horizontal_flip = {"horizontal_flip": True}
            labels["flags"].update(horizontal_flip)
            img = np.fliplr(img)
            instances.fliplr(w)
            # For keypoints
...
class RandomLoadText:
    ...
    def __call__(self, labels: Dict[str, Any]) -> Dict[str, Any]:
        ...
        labels["texts"] = texts
        labels["pos_labels"] = [label2ids[label] for label in pos_labels if label in label2ids]
        return labels
