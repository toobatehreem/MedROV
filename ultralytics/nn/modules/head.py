...
import cv2
from .dataset_matrix_with_labels import dataset
import open_clip
...
from ultralytics.utils.tal import TORCH_1_10, dist2bbox, dist2rbox, make_anchors, TaskAlignedAssigner
...
from ultralytics.data.deaugment import undo_flips, draw_bboxes, draw_pred_bboxes, undo_affine, reverse_bboxes, apply_bboxes, calculate_overlap, draw_bboxes_mosaic, transform_mosaic_coords, draw_transformed_coords,polygon_to_bbox, adjust_for_horizontal_flip
__all__ = "Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "v10Detect", "YOLOEDetect", "YOLOESegment"

biomed_model, biomed_preprocess = open_clip.create_model_from_pretrained('hf-hub:microsoft/BiomedCLIP-PubMedBERT_256-vit_base_patch16_224')
biomed_model.eval()
...
class WorldDetect(Detect):
    ...
    def __init__(self, nc=80, embed=512, with_bn=False, ch=()):
        ...
        self.cv5 = nn.ModuleList(nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch)
        self.proj = torch.arange(self.reg_max, dtype=torch.float, device=self.stride[0].device)
    ...
    def postprocess_preds(self, preds, agnostic=False):
    ...
    def bbox_decode(self, anchor_points, pred_dist, use_dfl, proj):
    ...
    def forward(self, x, text, batch, first_forward):
        proj = torch.arange(self.reg_max, dtype=torch.float, device=self.stride[0].device)
        ...
        if first_forward:
            if batch is not None:
                ...
                device=self.stride[0].device
                imgsz = torch.tensor(feats[0].shape[2:], device=self.stride[0].device, dtype=dtype) * self.stride[0]  # image size (h,w)
                scale_tensor=imgsz[[1, 0, 1, 0]]
                # Targets
                targets = torch.cat((batch["batch_idx"].view(-1, 1), batch["cls"].view(-1, 1), batch["bboxes"]), 1)
                targets = self.preprocess(targets.to(self.stride[0].device), batch_size, scale_tensor=imgsz[[1, 0, 1, 0]], device=self.stride[0].device)
                gt_labels, gt_bboxes = targets.split((1, 4), 2)  # cls, xyxy
                
                preds1 = x
                shape = preds1[0].shape  # BCHW
                x_cat = torch.cat([xi.view(shape[0], self.nc + self.reg_max * 4, -1) for xi in preds1], 2)
                anchors, strides = (x.transpose(0, 1) for x in make_anchors(preds1, self.stride, 0.5))
                pred_box, pred_cls = x_cat.split((self.reg_max * 4, self.nc), 1)

                dbox = self.decode_bboxes(self.dfl(pred_box), anchors.unsqueeze(0)) * strides

                final_preds0 = torch.cat((dbox, pred_cls.sigmoid()), 1)
                final_preds = self.postprocess_preds(final_preds0, agnostic=True) # preds after class agnostic NMS
                
                gt_boxes_np = gt_bboxes.cpu().numpy()  # Convert to numpy for batch processing
                gt_labels_np = gt_labels.cpu().numpy()  # Convert to numpy for batch processing
                ...
                for b_idx in range(batch_size):
                    counter = 0
                    patches = []
                    flags = batch.get('flags', [{}])[b_idx]
                    mosaic = flags.get('mosaic', False)
                    path = batch['im_file'][b_idx]
                    texts = batch['texts'][b_idx]
                    text_feats = batch['txt_feats'][b_idx]
                    pos_labels = batch['pos_labels'][b_idx]
                    gt_boxes_np_per_image = gt_boxes_np[b_idx]
                    img_tensor = batch['img'][b_idx].clone().cpu().numpy()
                    img = (img_tensor.transpose(1, 2, 0) * 255).astype(np.uint8)
                    preds_final = final_preds[b_idx].detach().cpu().numpy() # predcitions per image in a batch
                    preds_final = preds_final[preds_final[:, 4] >= 0.9]  # Filter predictions by confidence score >= 0.
                    
                    filtered_preds = []
                    for x1, y1, x2, y2, conf, cls_label in preds_final:
                        pred_box = torch.tensor([[x1, y1, x2, y2]], device=device)
                        gt_boxes_tensor = torch.tensor(gt_boxes_np_per_image, device=device)
                        if len(gt_boxes_tensor) > 0:
                            ious = bbox_iou(pred_box, gt_boxes_tensor).squeeze(0)
                            if ious.max().item() < 0.3:
                                filtered_preds.append([x1, y1, x2, y2, conf, cls_label])
                    
                    if len(filtered_preds) > 0:
                        new_final_preds = torch.tensor(filtered_preds, device=device)
                        new_final_preds = new_final_preds[torch.argsort(new_final_preds[:, 4], descending=True)]
                        if mosaic:
                            horizontal_flip = flags.get('horizontal_flip', False)
                            meta_data = batch['metadata'][b_idx]
                            if horizontal_flip:
                                # If mosaic and horizontal flip
                                model_preds = new_final_preds.clone()
                                un_flipped_image, un_flipped_bboxes = undo_flips(batch['img'][b_idx], model_preds, horizontal_flip=True)
                                unflipped_img_tensor = un_flipped_image.cpu().numpy()  
                                unflipped_img = (unflipped_img_tensor.transpose(1, 2, 0) * 255).astype(np.uint8)  # CHW -> HWC

                                # Reverse randomperspective transformation after unflipping
                                M = batch['M_matrix'][b_idx]
                                M_inv = np.linalg.inv(M)
                                
                                transformed_mosaic_coords = transform_mosaic_coords(meta_data, M, final_image_size=(640, 640))
                                
                                mosaic_coords_final = adjust_for_horizontal_flip(transformed_mosaic_coords)
                                
                                mosaic_coords_final_adjusted = []
                                for coords in mosaic_coords_final:
                                    mosaic_coords_final_adjusted.append({'mosaic_coords_final': [int(coords[0]), int(coords[1]), int(coords[2]), int(coords[3])]})
                                
                                # Get the image at mosaic level
                                ori_img = undo_affine(unflipped_img.copy(), M_inv, ori_shape=(1280, 1280))
                                un_flipped_bboxes_np = un_flipped_bboxes.clone().cpu().numpy()
                                ori_bboxes_np = apply_bboxes(un_flipped_bboxes_np, M_inv)

                                # Update the predictions to be at mosaic image level
                                ori_bboxes = torch.from_numpy(ori_bboxes_np).to(un_flipped_bboxes.device)
                                
                                matched_results = [] 
                                for ori_bbox, new_pred_bbox in zip(ori_bboxes.cpu().numpy(), new_final_preds.clone()):
                                    best_overlap = 0
                                    best_image = None
                                    
                                    for entry, entry1 in zip(meta_data, mosaic_coords_final_adjusted):
                                        mosaic_coords = entry['mosaic_coords']
                                        overlap = calculate_overlap(ori_bbox, mosaic_coords)

                                        # Keep track of the image with the highest overlap
                                        if overlap > best_overlap:
                                            best_overlap = overlap
                                            best_image = entry['img_name']
                                            mosaic_coords = entry['mosaic_coords']
                                            source_coords = entry['source_coords']
                                            mosaic_coords_final = entry1["mosaic_coords_final"]
                                            img_size = entry['img_size']

                                    # Store the result for this bbox
                                    if best_image:
                                        matched_results.append({
                                            "image_name": best_image,
                                            "ori_bbox": ori_bbox,
                                            "pred_bbox": new_pred_bbox,
                                            "overlap": best_overlap,
                                            "mosaic_coords": mosaic_coords,
                                            "source_coords": source_coords,
                                            "mosaic_coords_final" : mosaic_coords_final,
                                            "img_size": img_size
                                        })
                                    else:
                                        matched_results.append({
                                ...
                                for result in matched_results:
                                    if result["image_name"]:
                                        path = result["image_name"]
                                        parts = path.split('/')[-1].split('_')
                                        if parts[0] == "MSD":
                                            category = '_'.join(parts[:2])  # Combine the first and second parts
                                        else:
                                            category = parts[0]  # Default to the first part
                                                                                            
                                        pred_cls_id = int(result['pred_bbox'][-1]) # 1
                                        pred_class = texts[pred_cls_id] #rbc
                                                                                                                                
                                        matrix = np.array(dataset)
                                        row_index = np.where(matrix[:, 0] == category)[0]
                                        col_index = None  # Initialize a variable for the matched column index
                                        for idx in range(1, matrix.shape[1]):  # Start from 1 to skip the header row
                                            column_name = matrix[0, idx]
                                            if column_name is None:
                                                continue
                                            # Check if pred_class matches any of the slashed parts in the column
                                            if pred_class in column_name.split('/'):
                                                col_index = idx
                                                break
                                        if row_index.size > 0 and col_index is not None:
                                            value = matrix[row_index[0], col_index]
                                            if value == 0: # The class is not present in the dataset
                                                bbox_score = result['pred_bbox'][4]
                                                if bbox_score.item() >= 0.9:
                                                    batch_bboxes = batch['bboxes']
                                                    batch_classes = batch['cls']
                                                    batch_idx = batch['batch_idx']
                                                    new_batch_idx = torch.tensor([b_idx], dtype=batch_idx.dtype, device=batch_idx.device)

                                                    bbox_new = result['pred_bbox'][:4]
                                                    cls_new = int(result['pred_bbox'][-1])

                                                    reversed_bbox = xyxy2xywh(bbox_new).div_(scale_tensor)
                                                    reversed_bbox  = reversed_bbox.cpu().numpy()
                                                    
                                                    reversed_bbox_tensor = torch.tensor([reversed_bbox], device=batch_bboxes.device)

                                                    batch_bboxes = torch.cat((batch_bboxes, reversed_bbox_tensor), dim=0)
                                                    batch_classes = torch.cat((batch_classes, torch.tensor([[cls_new]], device=batch_classes.device)), dim=0)
                                                    batch_idx = torch.cat((batch_idx, new_batch_idx), dim=0)

                                                    batch['bboxes'] = batch_bboxes
                                                    batch['cls'] = batch_classes
                                                    batch['batch_idx'] = batch_idx                                                    
                                                else:
                                                    pass
                                                    # print("the score is not greater than 0.9")
                                            else: # The class is present in the dataset or can never be present
                                                if counter >=5:
                                                    continue
                                                else:
                                                    img_tensor = img.copy()
                                                    ...
                                                    bbox_new = bbox_new.cpu().numpy()
                                                    x1, y1, x2, y2 = bbox_new
                                                    bbox_width = x2 - x1
                                                    bbox_height = y2 - y1
                                                    
                                                    min_size, max_size = 50, 320
                                                    if bbox_width < min_size or bbox_height < min_size or bbox_width > max_size or bbox_height > max_size:
                                                        continue
                                                    else:
                                                        scale_factor = 0.30
                                                        delta_width = bbox_width * scale_factor
                                                        delta_height = bbox_height * scale_factor

                                                        x1_new = max(0, x1 - delta_width)  # Ensure it doesn't go outside the image
                                                        y1_new = max(0, y1 - delta_height)
                                                        x2_new = min(width, x2 + delta_width)
                                                        y2_new = min(height, y2 + delta_height)

                                                        bbox_new_resized = [x1_new, y1_new, x2_new, y2_new]
                                                        
                                                        mosaic_coords_final = result["mosaic_coords_final"]
                                                        mosaic_x1, mosaic_y1, mosaic_x2, mosaic_y2 = mosaic_coords_final
                                                        x1_resized, y1_resized, x2_resized, y2_resized = bbox_new_resized
                                                        
                                                        x1_resized = max(mosaic_x1, min(x1_resized, mosaic_x2))
                                                        y1_resized = max(mosaic_y1, min(y1_resized, mosaic_y2))
                                                        x2_resized = max(mosaic_x1, min(x2_resized, mosaic_x2))
                                                        y2_resized = max(mosaic_y1, min(y2_resized, mosaic_y2))
                                                        
                                                        resized_bbox_width = x2_resized - x1_resized
                                                        resized_bbox_height = y2_resized - y1_resized
                                                        min_size = 30  # Define a threshold for minimum size

                                                        if resized_bbox_width < min_size or resized_bbox_height < min_size:
                                                            continue
                                                        else:
                                                            bbox_new_resized1 = [x1_resized, y1_resized, x2_resized, y2_resized]
                                                            x1_patch, y1_patch, x2_patch, y2_patch = bbox_new_resized1
                                                            img_patch = img[int(y1_patch):int(y2_patch), int(x1_patch):int(x2_patch)]
                                                            patches.append(img_patch)
                                                            
                                                            bbox_new = result['pred_bbox'][:4]
                                                            reversed_bbox = xyxy2xywh(bbox_new).div_(scale_tensor).cpu().numpy()
                                                            reversed_bbox_tensor = torch.tensor([reversed_bbox], device=batch['bboxes'].device)

                                                            batch['bboxes'] = torch.cat((batch['bboxes'], reversed_bbox_tensor), dim=0)
                                                            batch['cls'] = torch.cat((batch['cls'], torch.tensor([[cls_new]], device=batch['cls'].device)), dim=0)
                                                            batch['batch_idx'] = torch.cat((batch['batch_idx'], torch.tensor([b_idx], dtype=batch['batch_idx'].dtype, device=batch['batch_idx'].device)), dim=0)
                                                            
                                                            counter += 1  
                                if patches:
                                    img_patches_pil = [Image.fromarray(patch) for patch in patches]
                                    img_processed_batch = torch.stack([biomed_preprocess(pil_patch) for pil_patch in img_patches_pil]).to(text_feats.device)
                                    img_processed_batch = img_processed_batch.to(dtype=text_feats.dtype)
                                    # with torch.no_grad():
                                    biomed_model.to(device=text_feats.device, dtype=text_feats.dtype)
                                    img_feats_batch = biomed_model.encode_image(img_processed_batch)

                                    for img_feats in img_feats_batch:
                                        valid_indices = set(range(self.nc)) - set(pos_labels)
                                        index = np.random.choice(list(valid_indices)) if valid_indices else 0
                                        pos_labels.append(int(index))
                                        text_feats[index] = img_feats.to(device=text_feats.device, dtype=text_feats.dtype)
                            else:
                                # If mosaic and no horizontal flip
                                model_preds = new_final_preds.clone()
                                M = batch['M_matrix'][b_idx]
                                M_inv = np.linalg.inv(M)
                                ...
                                mosaic_coords_final_adjusted = []
                                for coords in transformed_mosaic_coords:
                                    mosaic_coords_final_adjusted.append({'mosaic_coords_final': [int(coords[0]), int(coords[1]), int(coords[2]), int(coords[3])]})
                                ...
                                ori_img = undo_affine(img.copy(), M_inv, ori_shape=(1280, 1280))
                                ori_bboxes_np = model_preds.clone().cpu().numpy()
                                ori_bboxes_np = apply_bboxes(ori_bboxes_np, M_inv)
                                
                                ori_bboxes = torch.from_numpy(ori_bboxes_np).to(model_preds.device)
