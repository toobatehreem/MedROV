...
class WorldModel(DetectionModel):
    ...
    def get_text_pe(self, text, batch=80, cache_clip_model=True):
        ...
        if not getattr(self, "clip_model", None) and cache_clip_model:
            ...
            self.clip_model = open_clip.create_model_from_pretrained('hf-hub:microsoft/BiomedCLIP-PubMedBERT_256-vit_base_patch16_224')[0]
        model = self.clip_model if cache_clip_model else open_clip.create_model_from_pretrained('hf-hub:microsoft/BiomedCLIP-PubMedBERT_256-vit_base_patch16_224')[0]
    ...
    def predict(self, x, profile=False, visualize=False, txt_feats=None, batch=None, first_forward=None, augment=False, embed=None):
        """
        Perform a forward pass through the model.
        ...
        for m in self.model:  # except the head part
            ...
            if isinstance(m, C2fAttn):
                ...
                x = m(x, ori_txt_feats, batch, first_forward)
    ...
    def loss(self, batch, preds=None):
        ...
        if preds is None:
            self.eval()
            self.forward(batch["img"], txt_feats=batch["txt_feats"], batch=batch, first_forward=True)
                
            self.train()
            preds = self.forward(batch["img"], txt_feats=batch["txt_feats"], batch=batch, first_forward=False)
        return self.criterion(preds, batch)
